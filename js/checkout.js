...
function renderCart() {
    ...
    if (cart.length === 0) {
        ...
        $items.html('<div class="empty-cart"><i class="bi bi-cart-x fs-1"></i><p class="fw-bold mb-1">A kosarad üres</p><p class="text-secondary mb-3">Folytasd a vásárlást!</p><a href="webshop.html" class="btn i-web"><i class="bi bi-shop-window me-2"></i>Vásárlás folytatása</a></div>')
    ...
    cart.forEach(function(item, i) {
        ...
        html += '<div class="cart-item">' +
            '<img class="cart-item-img bg-white rounded-3" src="' + (item.image || 'assets/img/BraVa.webp') + '" alt="' + item.name + '">' +
            ...
            '</div></div>' +
            '<button class="remove-btn fs-5" onclick="removeItem(' + i + ')" title="Eltávolítás"><i class="bi bi-trash3"></i></button>' +
            '</div>'
    })
...
function applyPromo() {
    var cart = getCart()
    var kod = $('#promoInput').val().trim().toUpperCase()
    var $msg = $('#promoMsg')
    if (kod === 'BRAVA10') {
